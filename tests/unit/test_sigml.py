...
class TestSiGMLGenerator(unittest.TestCase):
    ...
    def test_generate_xml(self, MockRetriever):
        ...
        mock_retriever.get_hamnosys.side_effect = lambda g: f"ham_{g}" if g in ["HELLO", "WORLD"] else None

        generator = SiGMLGenerator()
        
        # Input phrase
        phrase = GlossPhrase(glosses=["HELLO", "WORLD"], original_text="Hello world")
    ...
    def test_missing_gloss(self, MockRetriever):
        ...
        generator = SiGMLGenerator()
        phrase = GlossPhrase(glosses=["HELLO", "UNKNOWN"], original_text="Hello unknown")
